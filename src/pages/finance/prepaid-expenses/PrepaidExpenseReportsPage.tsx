...
export default function PrepaidExpenseReportsPage() {
  ...
  const download = async () => {
    setDownloading(true);
    try {
      const res = await prepaidExpensesAPI.getReport(reportType, { format });
      if (format === "json") {
        ...
        a.href = url;
        a.download = `prepaid-${reportType}.json`;
        a.click();
        ...
        const a = document.createElement("a");
        a.href = url;
        a.download = `prepaid-${reportType}.${format}`;
        a.click();
        URL.revokeObjectURL(url);
      }
