...
export default function PrepaidExpenseSchedulesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await prepaidExpensesAPI.getAll({ limit: 100 });
      const expenses = res.data?.data?.data ?? res.data?.data ?? [];
      const detailResponses = await Promise.allSettled(
        expenses.slice(0, 50).map((e: { id: number }) => prepaidExpensesAPI.getById(e.id))
      );
      const flat: FlatLine[] = [];
      for (const r of detailResponses) {
        if (r.status !== "fulfilled") continue;
        const exp = r.value.data?.data;
        if (!exp?.scheduleLines?.length) continue;
        for (const line of exp.scheduleLines) {
          if (["POSTED", "CANCELLED", "REVERSED"].includes(line.postingStatus)) continue;
          flat.push({
            id: line.id,
            prepaidExpenseId: exp.id,
            prepaidNumber: exp.prepaidNumber,
                  ...
                  <TableRow key={line.id}>
                    <TableCell>
                      <Link
                        ...
                        to={`/finance/prepaid-expenses/${line.prepaidExpenseId}`}
                      >
                        {line.prepaidNumber}
                      </Link>
                    </TableCell>
