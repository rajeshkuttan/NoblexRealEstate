...
export default function LeaseRevenueDashboardPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await leaseRevenueAPI.getDashboard();
  ...
  const cards = [
    ...
    { label: t("dashboard.recognizedTotal"), value: kpis?.recognizedTotal ?? "0.00" },
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" asChild>
            <Link to="/finance/lease-revenue">{t("dashboard.viewRegister")}</Link>
          </Button>
          <Button asChild>
            <Link to="/finance/lease-revenue/posting-queue">{t("dashboard.viewQueue")}</Link>
          </Button>
        </div>
      </div>
