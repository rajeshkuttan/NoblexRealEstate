...
const broadcastAnnouncement = async (req, res, next) => {
  // Thin legacy wrapper — persists via building announcements API helper
  try {
    const buildingAnnouncementController = require('./buildingAnnouncementController');
    return buildingAnnouncementController.create(req, res, next);
  } catch (e) {
    next(e);
  }
};
